...
def _trim_to_longest_uninterrupted_block(df, nwb, verbose=False):
    """
    Mirror of get_session_tbl interruption logic using raw NWB times.
    Returns (trimmed_df, did_trim: bool). If no clean block found, returns (df, False).
    """
    ...
    right_reward_times = [None] * len(df)
    left_reward_times  = [None] * len(df)

    for i, row in df.iterrows():
        rr = right_rewards[
            (right_rewards <= row['reward_outcome_time'] + row['reward_delay'] + 0.1) &
            (right_rewards >  row['goCue_start_time'])
        ]
        lr = left_rewards[
            (left_rewards <= row['reward_outcome_time'] + row['reward_delay'] + 0.1) &
            (left_rewards >  row['goCue_start_time'])
        ]
        if len(rr) > 0:
            right_reward_times[i] = rr - row['goCue_start_time']
        if len(lr) > 0:
            left_reward_times[i] = lr - row['goCue_start_time']

    choice_time = df['reward_outcome_time'] - df['goCue_start_time']

    auto_manual_trials = [
        (
            (right_reward_times[i] is not None and
             np.any(right_reward_times[i] < choice_time.iloc[i] + df['reward_delay'].iloc[i]))
            or
            (left_reward_times[i] is not None and
             np.any(left_reward_times[i]  < choice_time.iloc[i] + df['reward_delay'].iloc[i]))
        )
    ...
    extra_reward_simple = [
        (
            (right_reward_times[i] is not None and bool(df['rewarded_historyR'].iloc[i]) is False)
            or
            (left_reward_times[i]  is not None and bool(df['rewarded_historyL'].iloc[i]) is False)
        )
        for i in range(len(df))
    ]

    # Build interruption mask; fill NAs as False to avoid astype(int) errors
    interruption = (np.array(auto_manual_trials) | np.array(extra_reward_simple))
    # If you WANT to keep the column:
    df["interruption"] = interruption

    # Convert to 0/1 list for longest_zero_chunk (0 = clean)
    binary = pd.Series(interruption, index=df.index).fillna(False).astype(int).tolist()

    longest_start, longest_end, longest_len = longest_zero_chunk(binary)
    if longest_start is None or longest_len == 0:
        return df, False

    trimmed = df.iloc[longest_start:longest_end + 1].reset_index(drop=True).copy()
    if verbose:
        print(f"Trimmed to uninterrupted trials [{longest_start}:{longest_end}] (n={longest_len})")
    return trimmed, True
...
def create_df_trials(nwb_filename, adjust_time=True, verbose=True, cut_interruptions=False):  # NOQA C901
    """
    ...
    if not np.all(earned_df["choice_time_in_session"] <= earned_df["reward_time_in_session"]):
        ...
        elif cut_interruptions:
            # try to fix the problem by trimming to the longest uninterrupted segment
            df, did_trim = _trim_to_longest_uninterrupted_block(df, nwb, verbose=verbose)
            if did_trim:
                # Recompute derived DataFrames used in the check on the trimmed df
                rewarded_df = df.query("earned_reward")
                earned_df = rewarded_df.query("not extra_reward")
                if not np.all(earned_df["choice_time_in_session"] <= earned_df["reward_time_in_session"]):
                    raise AssertionError("Reward before choice time (after trimming)")
            else:
                raise AssertionError("Reward before choice time (no uninterrupted block found)")
        else:
